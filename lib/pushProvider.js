...
exports.bookingClosedNotification = async (devices, payed, placeName) => {
  ...
  exports.sendCampaignAcceptedNotification = async (userCampaign, isAccepted) => {
    var note = getNotificationNote();
    note.alert = `\uD83D\uDCE7 \u2709 You have been ${isAccepted ? 'accepted' : 'rejected'} for the campaign!`;
    note.payload = { message: `Come and try it!`, campaign: userCampaign.campaign, isAccepted: isAccepted, pushType: 'campaignAccepted' };

    var message = {
      data: { campaign: userCampaign.campaign, isAccepted: isAccepted, pushType: 'campaignAccepted' },
      notification:{
          title : `You have been ${isAccepted ? 'accepted' : 'rejected'} for the campaign!`,
          body : 'Come and try it!'
      },  token : null };
      console.log('ustawienie note oraz message');
      console.log(userCampaign.user);
