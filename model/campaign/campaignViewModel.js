...
exports.toMobileViewModel = (campaigns, currentUser, withDetails) => {
    ...
    return campaigns.map(x=> {
      ...
      if(now.isAfter(availableFrom) && now.isBefore(availableTill)){
        ...
        x.isJoinable = x.daysToStart > 0 && x.users.filter(x=> x == currentUser._id).length == 0 && x.maxParticipantsCount - x.acceptedUsers.length > 0;
        ...
        x.participantsCount = x.maxParticipantsCount - x.acceptedUsers.length;
        delete x.startAt;
